...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminateVariable in eliminationOrder:
            currentFactors, joinedFactor = joinFactorsByVariable(currentFactors, eliminateVariable)
            if len(joinedFactor.unconditionedVariables()) == 1:
                continue
            
            currentFactors.append(eliminate(joinedFactor, eliminateVariable))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        values = [i[0] for i in sortedItems]
        rand = random.random()
        i, cum = 0, dist[0]
        while rand > cum:
            i += 1
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        for ithParticle in range(self.numParticles):
            self.particles.append(self.legalPositions[ithParticle % len(self.legalPositions)])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particlePos in self.particles:
            beliefs[particlePos] += self.getObservationProb(observation, pacmanPos, particlePos, jailPos)   
        beliefs.normalize()
